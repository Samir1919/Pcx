...
- UI and product flows must follow industry-standard patterns (e.g. an ecommerce product detail view uses a large main image with a thumbnail gallery and full specifications; an admin list shows human-readable product/condition/status, never raw UUIDs). Never ship a placeholder or minimal surface where a standard pattern exists.
