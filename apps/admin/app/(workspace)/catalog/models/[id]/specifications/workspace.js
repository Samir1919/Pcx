...
function Input({ definition, current }) {
  if (definition.dataType === "BOOLEAN") return <select name="value" defaultValue={display(current, "BOOLEAN")} required><option value="">Select value</option><option value="true">True</option><option value="false">False</option></select>;
  if (definition.dataType === "JSON") return <textarea name="value" defaultValue={display(current, "JSON")} placeholder='{"key":"value"}' required />;
  return <input name="value" type={definition.dataType === "NUMBER" ? "number" : "text"} step={definition.dataType === "NUMBER" ? "any" : undefined} defaultValue={display(current, definition.dataType)} required />;
}
...
export default function ModelSpecifications({ modelId }) {
  ...
  async function save(event, definition) {
    ...
    try {
      ...
      const value = parse(input, definition.dataType);
      const result = await catalogApi.setModelValue(modelId, definition.id, value);
      setValues((current) => [...current.filter((item) => item.specificationDefinitionId !== definition.id), { ...result.data, label: definition.label, key: definition.key, unit: definition.unit }]);
      setNotice({ kind: "success", message: `${definition.label} saved.` });
    } catch (error) {
          ...
          : definitions.length === 0 ? <div className="panel state">No active attribute definitions exist for this category.</div>
            : <section className="specGrid" aria-label="Model attributes">
              {definitions.map((definition) => {
                ...
                return (
                  <form className="panel specCard" key={definition.id} onSubmit={(event) => save(event, definition)}>
                    <div>
                    ...
                    <label><span>Value{definition.unit ? ` (${definition.unit})` : ""}</span><Input definition={definition} current={current?.value} /></label>
                    <div className="specFooter">
                      <small>{current ? `Updated ${new Date(current.updatedAt).toLocaleString()}` : "Not set"}</small>
                      <button className="primary" disabled={saving === definition.id}>{saving === definition.id ? "Saving…" : current ? "Update value" : "Set value"}</button>
                    </div>
                  </form>
                );
              })}
