...
export default function CatalogWorkspace() {
  ...
  async function edit(record) {
    const current = record.label ?? record.name;
    const next = window.prompt(`Update ${active === "definitions" ? "attribute label" : "name"}`, current);
    if (next == null || !next.trim()) return;
    let changes = active === "definitions" ? { label: next.trim() } : { name: next.trim() };
    if (active === "models") {
      const aliases = window.prompt("Update search aliases (comma separated)", (record.searchAliases ?? []).join(", "));
      if (aliases == null) return;
      changes = { ...changes, searchAliases: aliases.split(",").map((value) => value.trim()).filter(Boolean) };
    }
    if (JSON.stringify(changes) === JSON.stringify(active === "definitions" ? { label: current } : { name: current })) return;
    setBusy(true);
    try {
      await catalogApi.update(plural[active], record.id, changes);
