...
export const catalogApi = Object.freeze({
  ...
  adminModels: ({ cursor } = {}) => apiRequest(`/api/v1/admin/product-models?limit=50${cursor ? `&cursor=${encodeURIComponent(cursor)}` : ""}`),
  ...
  createBuild: (body) => apiRequest("/api/v1/admin/product-model-builds", { method: "POST", body }),
  createDefinition: (body) => apiRequest("/api/v1/admin/attribute-definitions", { method: "POST", body }),
  referenceValues: () => apiRequest("/api/v1/reference-values"),
  createReferenceValue: (body) => apiRequest("/api/v1/admin/reference-values", { method: "POST", body }),
  archiveReferenceValue: (id) => apiRequest(`/api/v1/admin/reference-values/${encodeURIComponent(id)}`, { method: "DELETE" }),
  compatibilityRules: () => apiRequest("/api/v1/compatibility-rules"),
  createCompatibilityRule: (body) => apiRequest("/api/v1/admin/compatibility-rules", { method: "POST", body }),
  archiveCompatibilityRule: (id) => apiRequest(`/api/v1/admin/compatibility-rules/${encodeURIComponent(id)}`, { method: "DELETE" }),
  ...
  setStatus: (resource, id, status) => apiRequest(`/api/v1/admin/${resource}/${encodeURIComponent(id)}/status`, { method: "PATCH", body: { status } }),
