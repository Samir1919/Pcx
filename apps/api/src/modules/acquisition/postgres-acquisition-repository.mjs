...
export function createPostgresAcquisitionRepository({ pool }) {
  ...
  return Object.freeze({
    ...
    async findAcquisitionById(acquisitionId) {
      const result = await pool.query(
        "SELECT id, sell_request_id, accepted_offer_id, seller_user_id, source_type, agreed_price, payment_status, ownership_confirmed_at, acquired_at, idempotency_key FROM acquisitions WHERE id::text = $1",
        [acquisitionId]
      );
      return result.rows[0] ? acquisition(result.rows[0]) : null;
    },
