...
import { createSellEntryConfig, hasPermission, Permission, parseBuildComponentRole, parseSellEntryIcon, parseSellEntryKey, parseSellEntryKind, sellEntryKeyFromSlug } from "@pcx/domain";
...
export function createSellTaxonomyService({ authService, readRepository, commandRepository, catalogService = null, id = randomUUID, clock = () => new Date() }) {
  ...
  if (!commandRepository || typeof commandRepository.updateEntry !== "function" || typeof commandRepository.updateComponent !== "function" || typeof commandRepository.createEntry !== "function") throw new TypeError("sell taxonomy command repository is required");
