// PostgreSQL repository for notification provider configuration.
//
// Mirrors the payment provider config repository: persists only the encrypted
// credentials blob per (provider, mode). The repository never sees plaintext
// credentials; it stores and returns the ciphertext produced by the credentials
// cipher. The service layer owns encryption, masking, and activation.
...
export function createPostgresNotificationProviderConfigRepository({ pool }) {
  if (!pool || typeof pool.query !== "function" || typeof pool.connect !== "function") throw new TypeError("PostgreSQL pool is required");

  return Object.freeze({
    async upsert(record) {
      const result = await pool.query(
        `INSERT INTO notification_provider_config(id, provider, mode, encrypted_credentials, active, created_at, updated_at)
