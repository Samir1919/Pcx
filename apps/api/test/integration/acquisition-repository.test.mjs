...
test("acquisition repository persists valuation/offer/immutable acquisition with idempotency", { skip: !connectionString }, async () => {
  ...
  const valuationId = "9a000000-0000-4000-8000-000000000004";
  ...
  try {
    ...
    await pool.query("DELETE FROM valuations WHERE id::text = $1", [valuationId]);
    ...
    const valuation = await repository.createValuation({ id: valuationId, sellRequestId: sellRequest, valuationType: "MANUAL", lowValue: 5000, highValue: 9000, recommendedValue: 7000, inputsSnapshot: null, createdBy: admin, createdAt: now });
    assert.equal(valuation.sellRequestId, sellRequest);

    const offer = await repository.createOffer({ id: offerId, sellRequestId: sellRequest, valuationId, amount: 7000, expiresAt: "2026-08-17T00:00:00.000Z", createdBy: admin, createdAt: now });
