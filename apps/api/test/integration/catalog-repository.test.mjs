...
test("catalog persistence enforces typed category alignment and deterministic public reads", { skip: !connectionString }, async () => {
  ...
  try {
    ...
    await pool.query("INSERT INTO spec_definitions(id,category_id,key,label,data_type,status) VALUES ($1,$3,'memory_gb','Memory','NUMBER','ACTIVE'),($2,$4,'socket_type','Socket Type','TEXT','ACTIVE')", [definitionA, definitionB, categoryA, categoryB]);
