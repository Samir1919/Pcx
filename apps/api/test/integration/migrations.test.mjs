...
test("identity migration is repeatable and installs security constraints", { skip: !connectionString }, async () => {
  ...
  try {
    ...
    for (const name of ["users", "roles", "permissions", "user_roles", "role_permissions", "addresses", "access_sessions", "refresh_families", "refresh_credentials", "auth_audit_events", "identity_action_tokens", "categories", "brands", "product_models", "spec_definitions", "model_spec_values", "sell_requests", "seller_declarations", "inventory_items", "serial_identifiers", "inspection_templates", "inspection_template_items", "valuations", "offers", "acquisitions", "listings", "listing_prices", "reservations", "orders", "order_items", "payments", "shipments", "shipment_events", "return_requests", "warranties", "claims", "claim_resolutions"]) assert.equal(names.has(name), true, `${name} is missing`);
