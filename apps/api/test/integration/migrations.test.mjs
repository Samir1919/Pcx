...
test("identity migration is repeatable and installs security constraints", { skip: !connectionString }, async () => {
  ...
  try {
    ...
    assert.deepEqual(migrations.rows.map(({ version }) => version), ["0001_identity_auth.sql", "0002_identity_policy_seed.sql", "0003_identity_action_tokens.sql", "0004_catalog.sql", "0005_catalog_admin_policy.sql", "0006_catalog_seed.sql", "0007_sell_requests.sql", "0008_inventory.sql", "0009_inspection_templates.sql", "0010_acquisition.sql"]);
    const tables = await pool.query("SELECT tablename FROM pg_tables WHERE schemaname = 'public'");
    const names = new Set(tables.rows.map(({ tablename }) => tablename));
    for (const name of ["users", "roles", "permissions", "user_roles", "role_permissions", "addresses", "access_sessions", "refresh_families", "refresh_credentials", "auth_audit_events", "identity_action_tokens", "categories", "brands", "product_models", "spec_definitions", "model_spec_values", "sell_requests", "seller_declarations", "inventory_items", "serial_identifiers", "inspection_templates", "inspection_template_items", "valuations", "offers", "acquisitions"]) assert.equal(names.has(name), true, `${name} is missing`);
