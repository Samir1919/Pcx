// ISO 3166-1 country metadata used by the storefront IntlPhoneInput.
// Source of truth for country codes, dial codes, and flag glyphs. The list is
// intentionally broad ("all countries") so the phone input is not BD-only.
...
// Strip hyphens from a dial code ("1-684" -> "1684") for prefix matching.
export function dialDigits(country) {
  return country.dial.replace(/-/g, "");
}

// Best-effort reverse lookup: given E.164 digits (no leading "+"), find the
// longest-matching country dial prefix and return that country plus the
// remaining national number. Falls back to null when nothing matches.
export function findCountryByDigits(digits) {
  ...
  for (const country of COUNTRIES) {
    ...
    if (!best || prefix.length > dialDigits(best.country).length) {
      best = { country, national: digits.slice(prefix.length) };
    }
  }
  return best;
}
