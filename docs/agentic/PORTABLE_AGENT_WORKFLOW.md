...
For customer web (`apps/web`) and admin web (`apps/admin`) UI changes, an HTTP 200 from `curl` is NOT sufficient evidence that a page works, and a real browser check is mandatory, not optional, before the change is reported as done. React runtime errors, console errors, and failed API requests happen client-side and are invisible to `curl`. Run `npm run web:check` (Playwright headless Chromium, `scripts/web-check.mjs`) against the running `npm run dev` stack; it loads the key web/admin pages and fails on page errors, `console.error`, failed requests, or missing content. When a test must also click through auth-gated flows, drive the browser with `--only web` / `--only admin` and extend `scripts/web-check.mjs` with the exact selectors, or use an MCP browser tool (e.g. Playwright MCP) if one is configured for the session.

When the slice changes a UI-browsable page or flow, the change is not "done" without committed headed-browser evidence. Leave `docs/verify/browser-verify.json` (schema in `docs/verify/README.md`) recording a human-like, start-to-end click-through of the full business flow for the changed surface, run in a real, visible window. Produce it with `PCX_HEADED=1 node scripts/business-e2e-check.mjs --evidence`, `PCX_HEADED=1 node scripts/admin-e2e-check.mjs --evidence`, `PCX_HEADED=1 node scripts/storefront-e2e-check.mjs --evidence`, or the Playwright MCP browser headed. `npm run ui-guard` (inside `npm run verify`) enforces this: a UI-browsable slice without valid headed evidence fails the gate.

If the required browser tooling is unavailable in the current environment (Playwright browsers not installed, or no MCP browser tool configured), do not skip verification and do not silently install or download anything. Ask the human for explicit permission to install/configure the missing tool (Playwright browsers via `npx playwright install`, or an MCP browser server), or, if that is not possible right now, report the UI change as verification-blocked with the exact reason. Never claim a browser-facing change works based on code review, `curl`, or a server log alone.

When verifying interactively through the Playwright MCP server (`.mcp.json`), the browser must run headed — a real, visible window — not headless. This repo's MCP config intentionally omits `--headless`; never add it, and never redirect the human to a screenshot-only or headless run when they ask to see the browser working. `npm run web:check` remains headless by design for the automated CI-style gate; that is a separate, non-interactive check and does not replace a visible MCP session when the human wants to watch or when the flow needs manual judgment.
