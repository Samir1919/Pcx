...
Re-verified 2026-09-01: several "remaining" items were confirmed complete and moved into verified scope (E1 admin user/role screens; E2 storefront integration; E3 admin indicative-price UI + media + notifications). The remaining work is rescheduled below in dependency order; the detailed slice breakdown lives in `docs/tasks/REMAINING_SCOPE_BACKLOG.md`.

1. **item_costs cost allocation (E4/E6)** — add the `item_costs` table and cost entries (ACQUISITION/REFURBISHMENT/TESTING/PACKAGING/SHIPPING_IN/OTHER), admin cost-entry UI, and surface allocation in pricing/reporting. Depends on existing acquisition_cost (migration 0036).
