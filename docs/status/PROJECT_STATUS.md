...
- Updated: 2026-08-31
- Current main evidence commit: `6d2a741` (RESERVED→SOLD on payment confirm, merged into `origin/main`)
...
1. The notification/contact delivery backlog (`docs/tasks/NOTIFICATION_DELIVERY_BACKLOG.md`) is now fully cleared: G (storefront IntlPhoneInput), H (shipment/order-delivery emit), I (provider-based MFA), and J (staging compose smoke) are complete. Dev runs Docker-first (`scripts/dev.mjs` + `infra/docker-compose.yml`) on Node 24 LTS.
