...
- Updated: 2026-08-18
- Current main evidence commit: `53cdaae` (fast-forward merge `agent/fix-admin-listings-prompt-modal`; admin listings publish/price actions use a `createPortal` modal instead of unsupported `window.prompt`, and duplicate active-listing/slug conflicts map to clean `409 LISTING_CONFLICT`)
...
| E15 — Notifications | In progress | Provider-neutral notification outbox (PENDING→SENT/FAILED) with SYSTEM_CONFIGURE-gated creation and dispatch; delivery failure never rolls back a business transaction; admin notification create workspace | Concrete email/SMS/push providers, retries, delivery visibility |
...
- Root `npm test`: 405 total, 383 pass, 0 fail, 22 skipped (DB integration) after the listings prompt-to-modal + publish-conflict-mapping slice.
