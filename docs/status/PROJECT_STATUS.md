...
3. ~~**Real bKash HTTP adapter (E10)**~~ — **COMPLETE 2026-09-01** (sandbox-only HTTP adapter + gateway for grant/create/execute/query/refund, LIVE fails closed). Remaining: webhook/IPN, redirect callback + reconciliation, bKash refund wiring into returns.
...
11. **E19 media** — ~~S3/MinIO swap~~ (done 2026-09-01; env-driven `s3-media-storage.mjs`), real malware gate (ClamAV).
12. **Bulk CSV import** for catalog models/attributes and indicative quote ranges — deferred; larger backend feature (parser + mapping + idempotent batch insert).
