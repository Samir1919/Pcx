...
- Updated: 2026-08-23
- Current main evidence commit: `9c505a3` (shipment/order customer notifications, merged into `origin/main`)
...
1. Scheduled notification/contact delivery follow-ups (J) are captured in `docs/tasks/NOTIFICATION_DELIVERY_BACKLOG.md`: staging compose smoke. G (storefront IntlPhoneInput), H (shipment/order-delivery emit), and I (provider-based MFA) are now complete. Dev runs Docker-first (`scripts/dev.mjs` + `infra/docker-compose.yml`) on Node 24 LTS.
