...
- Current engineering focus: E1 privileged MFA integration and authenticated self-service
...
| E1 — Identity, authentication & RBAC | In progress | Identity/RBAC contracts; customer/address records; auth/session and secure browser flows; audit/runtime/local limiter; complete provider-neutral contact-verification/password-reset persistence, application and HTTP flows | Production delivery adapter/distributed limits/atomic audit; privileged MFA integration; authenticated self-service; admin user/role screens |
...
- Root `npm run verify`: 50 unit/application tests pass; 4 PostgreSQL tests skip without `TEST_DATABASE_URL` by design.
...
- Latest detailed evidence: `docs/handoffs/E1_IDENTITY_ACTION_HTTP.md`.
...
1. E1 privileged MFA integration points.
2. E1 authenticated `/me` ownership boundary and address operations.
