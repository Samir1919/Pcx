...
| E1 — Identity, authentication & RBAC | In progress | Identity/RBAC contracts; auth/session and secure browser flows (storefront client transparent 15-minute access-token refresh — single-flight on 401, mirrors admin — so signed-in customers are not bounced to sign-in); audit/runtime/local limiter; contact/reset flows; privileged MFA gate and provider-neutral challenge verification; authenticated `/me`; ownership-safe authenticated address CRUD with origin/CSRF; provider-based privileged MFA (6-digit OTP via ContactDeliveryService, fail-closed to mfa_unavailable); surface-scoped session cookies for storefront vs admin (ADR 0013); admin user/role screens (list/filter + status/role change, permission-gated, audited) | Real MFA provider activation (hard stop); production delivery/distributed limits/atomic audit |
| E2 — Catalog & Product Model | In progress | Category/Brand/ProductModel contracts; typed specs; PostgreSQL persistence/runtime; audited admin catalog and typed specification-definition/value commands; responsive admin catalog and model-value UI with cursor-paginated product-model list; full-field edit modal (createPortal) for categories/brands/models/definitions; launch seeds and volume validation; safe typed specifications in public ProductModel detail; storefront listing search/listing integrated (E8) | Dedicated search index/recommendation |
| E3 — Sell-to-PCX | In progress | Owner-scoped authenticated sell-request create/list/get/submit with server-owned DRAFT, ownership declaration, DRAFT→SUBMITTED transition (web "Submit" now autosaves then submits), and sell-entry/build-component model (Desktop PC, PC Parts, Laptop, Laptop Parts); catalog taxonomy (`PC Parts`/`Laptop Parts` parent groups); public 4-entry sell flow with component build wizard and live indicative quote range (`GET /api/v1/quote-ranges`); admin-set server-owned indicative prices (`POST /api/v1/admin/indicative-prices`); admin sell-request queue excludes DRAFT + admin detail view (`GET /api/v1/admin/sell-requests/:id`); seller "My sell requests" view with status timeline + active offer Accept/Decline; profile autosave of sell-form name/phone; admin indicative-price UI (Quotes tab: category/model ranges + history); sell media upload + seller photo promote; SELL_REQUEST_SUBMITTED notification emit | — |
| E4 — Physical intake & inventory identity | In progress | Permission-gated physical intake as server-owned RECEIVED InventoryItem with database-enforced duplicate-identity rejection; server-derived human-readable PCX ID (`PCX-########`) generated from the item UUID (never client-authored); approved inspections record condition_grade + current_health_score + approved_at onto the item; admin inventory list/detail human-readable (product name, brand·category, condition grade, health, serial, acquisition cost); append-only per-item cost-allocation ledger (`item_costs`, ADR 0014) with server-derived total (acquisition seed + SUM(item_costs)) surfaced read-only in inventory detail and the operations report | warehouse/stock movement, full lifecycle UI |
| E5 — Inspection & verification | In progress | Versioned category-scoped inspection templates with typed, unique, canonical-code items created/read under SYSTEM_CONFIGURE; admin inspection template create form; technician inspection execution (`/api/v1/inspections` → start DRAFT, PUT results, submit) with server-derived rule-based health score (critical=3× weight), suggested grade (A_PLUS/A/B/C/REJECT), mandatory-test enforcement, SUBMITTED/ESCALATED state, supervisor approve/reject recording grade/health onto the inventory item, admin inventory "Inspect" modal, and inspection evidence upload (`POST /api/v1/inspections/:id/media`, PRIVATE, technician-gated); reinspection/supersede (a new inspection supersedes a SUBMITTED/ESCALATED inspection, history preserved) and reasoned, audited supervisor override of a critical-fail (ESCALATED → APPROVED requires grade + reason, `POST /api/v1/inspections/:id/override`) | technician autosave/draft polish |
| E6 — Acquisition, cost & final offer | In progress | Final offer lifecycle created directly from a sell request (ACTIVE→ACCEPTED/REJECTED with expiry; internal valuation concept removed via ADR 0012), seller-owned public accept/reject endpoints (`POST /api/v1/offers/:id/accept` and `/reject`, ownership enforced), immutable idempotent acquisition (agreedPrice derived from accepted offer), and server-owned acquisition payment transition (PENDING→PAID via `POST /api/v1/admin/acquisitions/:id/pay`); admin acquisition workflow workspace; admin acquisition continuation (offer history list, seller-agreed accept, dynamic acquisition creation, mark-paid); sell-request status auto-advance across offer→accept→inspection→acquisition→pay (inspection reordered after acceptance, ADR 0016); acquisition-to-inventory intake (RECEIVED) with server-derived acquisition cost (migration 0036); append-only per-item cost allocation (item_costs, ADR 0014) | real payment gateway |
| E7 — Listing, pricing & passport | In progress | Server-owned listing lifecycle (DRAFT→PUBLISHED→RESERVED), versioned asking-price history, one-active-listing-per-item constraint, and safe public passport projection (`GET /passport/:pcxId`) excluding serial/cost/private evidence; public listing cards + passport now surface the server-derived condition grade and health score for published items; admin listing management workspace + `GET /api/v1/admin/listings` (draft create, publish, price set); listing photo upload (`POST /api/v1/admin/listings/:id/media`, PUBLIC, admin-gated); order creation atomically moves the sellable listing PUBLISHED→RESERVED so a purchased item is no longer public/buyable; admin inventory per-item List action creates a draft listing (intake → listing bridge, verified headed through publish/price → storefront); public listing cards + passport surface cover/gallery media (promoted seller photos); admin listing table human-readable (brand·category, condition grade, labeled status, formatted ৳ price); draft creation gated to APPROVED inventory items (spec invariant "listing = approved item offer" enforced, ITEM_NOT_APPROVED); payment confirm marks the claimed listing SOLD (RESERVED→SOLD); server-derived verification summary and a scan-to-open passport QR on the public passport | — |
| E8 — Search, discovery & storefront | In progress | Public storefront listing search (`GET /api/v1/listings`) with allow-listed query params, cursor pagination, and safe disclosure-only listing cards (no serial/cost/private evidence); responsive storefront UI shell (`apps/web`) with category/brand filter, sort, cursor pagination, and public passport page; storefront card + passport now render listing cover/gallery images, human grade label (A+), and a full ecommerce product detail view (main image + thumbnail gallery + full specifications) | recommendation/dedicated search index |
| E9 — Cart, reservation & checkout | In progress | Bounded reservation with database-enforced one-active-per-item constraint (double-sell guard), customer-gated create/convert/read-active, concurrency-proof integration, a worker background job (`expireDue`) that expires passed ACTIVE reservations, and persistent customer carts (`/api/v1/cart` add/get/remove) with server-owned price snapshot from the published listing; order creation consumes the ACTIVE reservation exactly once (ACTIVE→CONVERTED) and claims the listing atomically, so the DB is the final authority against double-selling; payment confirm advances the order (PENDING_PAYMENT→CONFIRMED) and marks each claimed listing SOLD (RESERVED→SOLD); server-derived shipping/tax allocation on orders (flat shipping free above threshold + 5% VAT, migration 0039) rendered read-only in the customer order breakdown | — |
| E10 — Order & payment | In progress | Customer-gated order creation with server-computed totals and sold-fact snapshots; idempotent payments with server-derived provider transaction ids; COD (Cash on Delivery) as a first-class, gateway-free, idempotent payment method (stays INITIATED until delivery collection); bKash via the injected sandbox gateway with server-authoritative provider identity; admin panel stores sandbox/live bKash credentials encrypted at rest (AES-256-GCM); real bKash HTTP adapter (sandbox-only) — grant token / create payment (URL-based "0011") / execute / query / refund with researched endpoints and sandbox-host guard; LIVE mode fails closed (hard stop) | — |


| E11 — Fulfilment & shipment | In progress | Server-owned shipment lifecycle (DRAFT→SHIPPED→DELIVERED→RETURNED) with unique tracking id and persisted shipment events, gated by INVENTORY_MANAGE/SYSTEM_CONFIGURE; tracking id is server-authoritative, derived from the injected sandbox courier; signed courier webhook (`POST /api/v1/webhooks/courier`) advances DELIVERED/RETURNED with timing-safe secret validation and idempotent final-state handling; durable courier webhook outbox (`shipment_webhook_events`) enqueues every webhook before application and a worker job (`dispatchDueWebhookEvents`) retries PENDING events with a bounded backoff budget until APPLIED or FAILED; admin shipment management workspace with read-only shipment list; packaging evidence media (`shipment_media` link + `POST/GET /api/v1/admin/shipments/:id/media`, PRIVATE, admin-gated upload) and return-to-origin admin action (`POST /api/v1/admin/shipments/:id/return`, SHIPPED→RETURNED) | — |



| E12 — Return & refund | In progress | Customer-gated return request with server-owned REQUESTED→APPROVED→RECEIVED→REFUNDED lifecycle and database-enforced one-refundable-request-per-item (double-refund guard); physical serial-match enforced on return intake (received serial must equal the sold unit's primary serial, normalized); admin return & refund management workspace with read-only return list; sandbox refund gateway (`createSandboxRefundGateway`) wired behind settleRefund with idempotent server-derived provider transaction id, replay-safe REFUNDED transition, and gateway failure never rolling back the REFUNDED transition (FAILED provider status recorded for reconciliation) | carrier pickup |
| E13 — Warranty & claims | In progress | One warranty per sold order item with a valid window, server-owned claim lifecycle (REQUESTED→RESOLVED) with typed resolutions (REPAIR/REPLACE/REFUND/REJECT), and customer-owned public claim creation (`POST /api/v1/claims`, ownership enforced); admin warranty & claims management workspace with read-only warranty and claim lists; server-authored warranty policies (`warranty_policies`, `POST/GET /api/v1/admin/warranty-policies` + archive, SYSTEM_CONFIGURE, with snapshot helper `toWarrantyPolicySnapshot`) and admin authoring panel; claim inspections (`POST /api/v1/admin/claims/:id/inspection`, REQUESTED→IN_REVIEW, `linkClaimInspection`, migration 0042); claim carrier pickup (`POST /api/v1/admin/claims/:id/shipment`, `linkClaimShipment`, migration 0043); cost accounting via resolution `costAmount` | — |
| E14 — Admin operations & reporting | In progress | Admin-gated operations dashboard (`GET /api/v1/admin/reports/operations`) with lifecycle counts and recent orders/sell requests under AUDIT_READ/SYSTEM_CONFIGURE; BI dashboard (`GET /api/v1/admin/reports/bi`) with server-derived revenue/inventory KPIs; CSV export (`GET .../operations/export?format=csv`); scheduled exports (`scheduled_exports`, migration 0045, worker `runDue`); admin operational workspaces + Reports workspace | Per-module operational screens |
| E15 — Notifications | In progress | Provider-neutral notification outbox (PENDING→SENT/FAILED) with SYSTEM_CONFIGURE-gated creation and dispatch; delivery failure never rolls back a business transaction; admin notification create workspace with read-only notification list; EMAIL (Resend) + SMS (bdBulksms) provider credential config in admin (masked, encrypted, sandbox/live activation); synchronous contact delivery for verify/reset OTP + provider-based MFA OTP; deterministic event emitter + idempotent outbox with ORDER_PLACED/OFFER_CREATED/SELL_REQUEST_SUBMITTED emits; storefront IntlPhoneInput + mirrored client-side contact validation; SHIPMENT_SHIPPED + ORDER_DELIVERED customer emits with composition-root order→user resolver (commerce `getUserIdByOrder`) | Real provider activation |
| E16 — Audit, observability & jobs | In progress | Append-only audit logs (`audit_logs`) with AUDIT_READ-gated filtered listing and best-effort critical-action writes (listing publish, price change, inspection approve/reject); no auto-delete retention; external-SIEM NDJSON export (`GET /api/v1/admin/audit-logs/export?format=ndjson`); liveness/readiness endpoints | — |
| E17 — Security hardening | In progress | Baseline response security headers (`nosniff`, `DENY`, `no-referrer`, restrictive CSP, HSTS, `permissions-policy`) with regression coverage; browser-facing Next.js apps (admin + storefront) serve a CSP allow-list (`default-src 'self'`, inline script/style, `img-src data: blob:`, `frame-ancestors 'none'`) plus HSTS/nosniff/DENY/referrer via `next.config.mjs`; upload malware scanning (fail-closed signature scanner, `MALWARE_DETECTED` 422) | MFA gates (real MFA = hard stop) |
| E18 — Backup, staging & release readiness | In progress | Release preflight (`npm run release:preflight`) verifying staging/backup/restore artifacts and no placeholder secrets; full production-like staging compose smoke (`npm run staging:smoke`: api/web/admin healthy, isolated `pcx-staging` project); runbook in handoff | Real production deployment and real secrets (hard stop) |
| E19 — Media & evidence | In progress | Local/NFS-backed image media module (`MEDIA_ROOT`-driven, default `apps/api/uploads`); S3/MinIO object-storage adapter (`s3-media-storage.mjs`, env-driven via `OBJECT_STORAGE_*`, local fallback); `media` + link tables (`sell_request_media`, `inspection_media`, `listing_media`); storage adapter with magic-byte MIME allow-list, 5 MiB limit, server-generated keys, path-traversal guard; public/private visibility access control; multiple-image client upload UI (admin listing "Photos" modal, sell-request "Item photos", inspection "Evidence" file input) with real-browser checks; seller photo pick-and-promote (PRIVATE→PUBLIC flip + listing_media link) so seller photos reach the storefront | — |
...
- Root `npm test`: 569 total, 542 pass, 0 fail, 27 skipped (DB integration) after the valuation-removal slice.
...
- SHIPMENT_SHIPPED / ORDER_DELIVERED customer emits (commit `f756397`): shipment service takes an `orderUserResolver` (composition-root commerce `getUserIdByOrder` method, no raw cross-module query) + `notificationEmitter` and emits best-effort PENDING rows on ship/deliver (admin action, courier webhook, and worker dispatch paths). `npm run verify` pass (554 tests / 527 pass / 0 fail / 27 skipped); integration test order→ship→deliver→PENDING rows passed against local Postgres.
...
Re-verified 2026-09-01: several "remaining" items were confirmed complete and moved into verified scope (E1 admin user/role screens; E2 storefront integration; E3 admin indicative-price UI + media + notifications). item_costs cost allocation (E4/E6) completed 2026-09-01 (ADR 0014). The remaining work is rescheduled below in dependency order; the detailed slice breakdown lives in `docs/tasks/REMAINING_SCOPE_BACKLOG.md`.

1. ~~**item_costs cost allocation (E4/E6)**~~ — **COMPLETE 2026-09-01** (ADR 0014; `item_costs` ledger + admin multi-field cost entry + server-derived totals in inventory detail and operations report).
2. ~~**Refund gateway adapter execution (E12, sandbox)**~~ — **COMPLETE 2026-09-01** (`createSandboxRefundGateway` + `refund_provider*` columns on `return_requests`; settleRefund is replay-safe and gateway failure never rolls back REFUNDED).
3. ~~**Real bKash HTTP adapter (E10)**~~ — **COMPLETE 2026-09-01** (sandbox-only HTTP adapter + gateway for grant/create/execute/query/refund, LIVE fails closed; redirect callback + server-to-server IPN reconciliation; bKash refund wiring into returns via migration 0046 `payments.provider_trx_id` + `getRefundContextByOrder`).
4. ~~**E5 inspection follow-ups**~~ — **COMPLETE 2026-09-01** (reinspection/supersede + reasoned audited supervisor override of critical-fail; technician autosave was already in place via incremental result upsert).
5. ~~**E7/E8 passport & storefront**~~ — **PARTIAL COMPLETE 2026-09-01** (verification summary + passport QR done). Remaining: dedicated search index/recommendation.
6. ~~**E9 order/payment allocation**~~ — **COMPLETE 2026-09-01** (server-derived shipping/tax on orders, migration 0039, read-only customer breakdown).
7. ~~**E11 fulfilment**~~ — **COMPLETE 2026-09-01** (packaging evidence media `shipment_media` + admin upload, and return-to-origin `POST /api/v1/admin/shipments/:id/return`).
8. ~~**E13 warranty**~~ — **COMPLETE 2026-09-01** (policy authoring + claim inspections + carrier pickup; cost accounting via resolution costAmount).
9. ~~**E14/E16 reporting & audit**~~ — **COMPLETE 2026-09-01** (BI dashboard, CSV export, external-SIEM NDJSON export, scheduled exports + worker job).
10. **E17 security** — ~~upload scanning~~ (done 2026-09-01), ~~HSTS~~ (done), ~~CSP allowlisting for admin UI~~ (done 2026-09-01), MFA gates (hard stop).
11. ~~**E19 media**~~ — **COMPLETE 2026-09-01** (S3/MinIO swap via env-driven `s3-media-storage.mjs`, and real ClamAV clamd malware gate via `clamav-scanner.mjs` — opt-in `CLAMAV_ENDPOINT`, fail-closed composite with signature fallback, optional `clamav` docker-compose service).
12. ~~**Bulk CSV import**~~ — **COMPLETE 2026-09-01** (`POST /api/v1/admin/catalog/import` for catalog models + indicative quote ranges; idempotent by slug; admin Import CSV tab; headed browser evidence).
...
## Catalog Attributes (specification definitions) hardening (2026-09-04)

- ~~**Required enforcement + atomic bulk model-spec save + passport consistency**~~ — **COMPLETE 2026-09-04** (commit `0a7d061`): the `required` flag on attribute definitions is now server-enforced — a model's submitted value set must include every `required` definition (domain `assertRequiredSpecificationValues`), written all-or-nothing in one transaction via bulk `PUT /api/v1/admin/product-models/:id/specifications`. Admin model-spec page is a single bulk form with a "N of M required attributes set" completeness banner. Public passport listing repo now filters archived definitions (`d.status='ACTIVE'`).
- ~~**Storefront attribute filters (layered navigation)**~~ — **COMPLETE 2026-09-04** (commit `f3a9480`): the `filterable` flag is now functional. `GET /api/v1/listings` accepts `spec[key]=value` filters (EXISTS constraints on model_spec_values) and new `GET /api/v1/listings/facets` returns filterable keys + distinct values. Marketplace renders a facet filter panel (label + values + units). Matches Magento/WooCommerce layered-navigation pattern.
- Remaining (non-blocking): reusable/global attributes (attribute sets) + enum/select data types.
- ~~**Selected-specs sell-flow capture**~~ — **COMPLETE 2026-09-04** (commit `2c4dd6f`): the dead `selected_specs` field is revived — the server now snapshots the picked variant model's published typed specs (TEXT/NUMBER/BOOLEAN; JSON skipped) as the seller-declared selected_specs, resolved server-side (never client-trusted) via the catalog public read. Declaration stays non-authoritative for price/grade/health.
...
## Sell flow runtime configuration (2026-09-03)
