...
| E3 — Sell-to-PCX | In progress | Owner-scoped authenticated sell-request create/list/get/submit with server-owned DRAFT, ownership declaration, and DRAFT→SUBMITTED transition; read-only admin sell-request queue (`GET /admin/sell-requests`) surfaced in acquisition workspace | Estimated range, info/inspection/valuation/offer flows, media, notifications |
