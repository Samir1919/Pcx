...
| E9 — Cart, reservation & checkout | In progress | Bounded reservation with database-enforced one-active-per-item constraint (double-sell guard), customer-gated create/convert/read-active, concurrency-proof integration, and a worker background job (`expireDue`) that expires passed ACTIVE reservations to release the physical item | Cart persistence (full cart entity), order/payment allocation |
...
- Root `npm test`: 476 total, 453 pass, 0 fail, 23 skipped (DB integration) after the inspection-execution, server-derived PCX ID, and COD payment slices.
