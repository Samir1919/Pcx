...
- Root `npm test`: 554 total, 527 pass, 0 fail, 27 skipped (DB integration) after the shipment/order-delivery emit slice.
...
1. Scheduled notification/contact delivery follow-ups (I/J) are captured in `docs/tasks/NOTIFICATION_DELIVERY_BACKLOG.md`: provider-based MFA, staging compose smoke. G (storefront IntlPhoneInput) and H (shipment/order-delivery emit) are now complete. Dev runs Docker-first (`scripts/dev.mjs` + `infra/docker-compose.yml`) on Node 24 LTS.
